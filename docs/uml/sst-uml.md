SST UML — UI, React Components, and External Services

This document captures how the SST Next.js PWA interacts across the UI, React components, client-side services, and external Google APIs, within the MVP constraints (manual sync, no backend DB, offline-first).
...
sequenceDiagram
  ...
  alt User chooses Re-create here
    ...
    Ensure->>GD: create folder under /My Drive/SST
...
- Drive layout: `/My Drive/SST/<ProjectName>__<projectId>/`; `project.json` is written last.
