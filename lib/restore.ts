...
async function restoreSingleProject(projectInfo: DriveProjectInfo): Promise<void> {
    ...
    // Restore pins and their photo metadata
    ...
    for (const pinData of pins) {
        ...
        try {
            ...
            // Restore photo metadata (but NOT the actual image data)
            // The actual images will be lazy-loaded from Drive when needed
            ...
            for (const photoData of photos) {
                ...
                try {
                    ...
                    const photoRow: PhotoRow = {
                        ...
                        id: photoData.id,
                        pinId: pinData.id,
                        localUri: '', // Empty initially, will be lazy-loaded from Drive
                        width: photoData.width || 0,
                        height: photoData.height || 0,
                        sizeBytes: photoData.sizeBytes || 0,
                        driveFileId: photoData.driveFileId,
                        status: photoData.status || 'synced',
                    }
                    await db.photos.add(photoRow)
                } catch (error) {

                    logger.error('Failed to restore photo metadata', error, { photoId: photoData.id })
